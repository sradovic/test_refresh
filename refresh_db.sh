...
dest_backup (){
bash pullsite -i ${DEST_IMAGE} -k  ${dst_db_user}  -t ${DEST_SITE} -D /bmsrv99${DEST_DC}${DEST_TY}/oracle/backup/datapump_ondemand -l ${NO_PARALL} -v -M prod ${PPARTIAL} 2>&1 &
EXPPID="$!"
echo " PID: $EXPPID"
export $EXPPID
}
...
source_backup (){
bash pullsite -i ${SOURCE_IMAGE} -k  ${souce_db_user}  -t ${SOURCE_SITE} -D /bmsrv99${SOURCE_DC}${SOURCE_TY}/oracle/backup/datapump_ondemand -l ${NO_PARALL} -v -M prod ${PPARTIAL}
}
...
bash deployimage --tassubid "${TASID}" --companyname "${SHORT_SITENAME}"  --cpqdbnode "${dbhost_mapped}" --passwordenc "${dst_db_passwd}" --image "${SOURCE_IMAGE}" --mapkey "${dst_db_user}" --target "${IPETH1}" --cpqdbuser "${dst_db_user}"  --jvmmempool ${Xms} --gcnewsize ${XX}  --mode prod --propman restore --cpqnode node1 --registryname "${REGISTRY_NAME}" ${BIG_USER} ${DPARTIAL} ${Start} ${Stop}
}


Deploy_to_Target_18_D_using_par_file (){

bash deployimage --tassubid "${sub_id}" --companyname "${companyname}"  --cpqdbnode "${cpqdbnode}" --passwordenc "${passwordenc}" --image "${image}" --mapkey "${mapkey}" --target "${target}" --cpqdbuser "${cpqdbuser}"  --jvmmempool ${Xms} --gcnewsize ${XX}  --mode prod --propman restore --cpqnode node1 --registryname "${REGISTRY_NAME}" ${BIG_USER} ${DPARTIAL} ${Start} ${Stop}
}
...
exec 2>"logs/refresh/refresh${DEST_SITE}$DATE1.log"
...
case ${TYPEOFDEPLOYMENT} in
    1 )
       ...
       echo " Deployment using parametar file";
                        if [[ $BATCH == no ]]
                          echo ""
                          echo "bash deployimage --tassubid "${sub_id}" --companyname "${companyname}"  --cpqdbnode "${cpqdbnode}" --passwordenc "${passwordenc}" --image "${image}" --mapkey "${mapkey}" --target "${target}" --cpqdbuser "${cpqdbuser}"  --jvmmempool ${Xms} --gcnewsize ${XX}  --mode prod --propman restore --cpqnode node1 --registryname "${REGISTRY_NAME}" ${BIG_USER} ${DPARTIAL} ${Start} ${Stop}" 
                          echo "" 
                                then
    ...
    2 )
       ...
       echo " Deployment using info form existing site";
                        if [[ $BATCH == no ]]
                                then
                                 echo ""
                                 echo "bash deployimage --tassubid "${TASID}" --companyname "${SHORT_SITENAME}"  --cpqdbnode "${dbhost_mapped}" --passwordenc "${dst_db_passwd}" --image "${SOURCE_IMAGE}" --mapkey "${dst_db_user}" --target "${IPETH1}" --cpqdbuser "${dst_db_user}"  --jvmmempool ${Xms} --gcnewsize ${XX}  --mode prod --propman restore --cpqnode node1 --registryname "${REGISTRY_NAME}" ${BIG_USER} ${DPARTIAL} ${Start} ${Stop}"
                                 echo ""
                                     printf "Do yo want to deploy image: (y/n) \n";
                                read x ;
                        fi;
